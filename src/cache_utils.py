...
def compute_source_fingerprint(paths: List[str]) -> str:
    ...
    Si un seul octet change dans un des fichiers, l'empreinte change.
    Permet de détecter qu'un modèle pickle/joblib est périmé sans avoir
    à comparer manuellement des timestamps (peu fiables sur certains
    systèmes de fichiers / après un git clone).

    Parameters
    ----------
    paths : List[str]
        Chemins absolus des fichiers dont dépend le modèle.

    Returns
    -------
    str
        Empreinte hexadécimale du contenu combiné des fichiers.
    """
...
def is_cache_valid(model_data: Dict[str, Any], source_paths: List[str]) -> bool:
    ...
    model_data : Dict[str, Any]
        Dict chargé depuis disque, doit contenir la clé "source_fingerprint".
    source_paths : List[str]
        Fichiers source actuels à vérifier.

    Returns
    -------
    bool
        True si le cache est encore valide, False s'il faut reconstruire.
    """
